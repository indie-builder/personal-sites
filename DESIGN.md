...
- 刊头承载全部内容入口（每日动态/每日关注/设计收藏/抖音收藏/开源关注/问一问）：问一问虽是对话式动作，仍与阅读版块等权出现在版块序列末位；`/ask` 页的刊名是"问一问"，兄弟位与其余栏目页一致。
...
- 检索范围在组合器底部左侧（`DropdownMenu`）：触发器显示当前范围，点击向上展开单选列表（全部/关于我/每日动态/每日关注/开源关注）；不使用平铺的分段控件。
